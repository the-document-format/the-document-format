...
pub trait Store<B: Backend>: StoreTypes {
    ...
    fn iter<'a>(
        &self,
        backend: &'a B,
    ) -> Box<dyn Iterator<Item = &'a StoreItemCell<Self, B>> + 'a>
    where
        B: BackendAccess<Self, B>;
}
...
pub trait StoreExt<B: Backend>: Store<B> {
    fn iter_rec(
        ...
        pointer: &BackendPointer<Self, B>,
        backend: &B,
    ) -> Vec<(Self::Primitive, Self::Unique)>
    where
        B: BackendAccess<Self, B>;
    fn iter_range_rec(
        &self,
